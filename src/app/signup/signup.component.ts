...
export class SignupComponent implements OnInit {
  userTypes = [
    ...
    {display: 'Shop Owner', data: 'shopOwner'},
    {display: 'Agent', data: 'agent'},
  ];
  email: string;
  phonenum: number;
  firstName: string;
  ...
  getLocation() {
    navigator.geolocation.getCurrentPosition(
      ...
      error =>
        (this.locationError = {message: 'location fetching not working'}),
      {timeout: 10000}
    );
  }
  ...
  validate(): any {
    ...
    if (!this.lastName)
      return {status: false, message: 'Last name cannot be empty'};
